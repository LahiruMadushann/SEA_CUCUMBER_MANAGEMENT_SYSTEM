...
export default function ViewSingleProcessedRecScreen() {
  ...
  useEffect(() => {
    ...
    async function fetchSingleProcessedDetails() {
      try {
        const response = await axios.post(`${BASE_URL}/processer/getSingleProcessedDetails`, { recordId: recordId });
  ...
  const stockImageUrls = imageFileNamesArray.map((fileName) => `${BASE_URL_FOR_STOCK_PICS}/${fileName}`);
            ...
            <View className="mt-[58vh] ">
              <View className="flex-row ">
                <View className=" ml-[4vw]">
                  <TouchableOpacity onPress={() => navigation.navigate(backNav)}>
              ...
              <Text className="text-center text-[#fff] font-bold text-[22px] mt-[10vw] fixed">Processed Records</Text>
