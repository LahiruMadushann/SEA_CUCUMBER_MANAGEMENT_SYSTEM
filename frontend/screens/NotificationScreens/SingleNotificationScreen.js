 ...
 // const decodedToken = jwtDecode(token);
  ...
  useEffect(() => {
    ...
    async function fetchSingleNotificationData() {
      try {
        const response = await axios.post(`${BASE_URL}/user/getSingleNotification`, { notificationId: notificationId });
  ...
  const handleDelete = async () => {
    ...
    Alert.alert("Are you sure?", "Once you delete your News, you won't be able to recover it.", [
      {
        text: "Delete News",
        onPress: () => {
          const newsData = {
          ...
          axios
            ...
            .catch((error) => {
              ...
              Alert.alert("Error", "An error occurred while deleting the Advertisement.");
            });
          ...
          <View className="mt-[20vh] mx-auto">
            ...
            <View className="w-[82vw] h-[auto] rounded-[30px] bg-[#FFFFFF] shadow-lg shadow-gray-700 mb-2">
              <View className="w-[230px] h-[25px] ml-[-4vw] mt-[4vw] flex-row ">
                <Image source={require("../../assets/notification/calender.png")} className="w-[13px] h-[18px] ml-[10vw]" />
                <Text className="text-[14px] font-bold text-[#000000A6] ml-[2vw]">{`${formatDate(db_date)} at ${formatTime(db_date)}`}</Text>
              </View>
              <View className="flex">
                <Text className=" text-[14px] text-center font-bold flex-auto mt-[2vw] ml-[10vw] mr-[10vw]">{db_title}</Text>
              </View>

              <View className="flex">
                <Text className=" text-[11px] text-justify flex-auto mt-[0vw] ml-[10vw] mr-[10vw]">{db_description}</Text>
              </View>
              ...
              {db_type === "RulesAndRegulations" && (
                <View className="flex mx-[30vw] mb-[5vw] w-[200px] ">
                  <Image source={require("../../assets/notification/rulesAndRegulations.png")} className="w-[15px] h-[18px] ml-[11vw] mt-[1vw]" />
                  <Text className=" text-[11px] text-[#ff0000] font-bold flex-auto mt-[-5vw] ml-[17vw]">Rules and Regulations</Text>
                </View>
              )}

              {db_type === "SeacucumberRates" && (
                <View className="flex mx-[30vw] mb-[5vw] w-[200px] ">
                  <Image source={require("../../assets/notification/price-list.png")} className="w-[15px] h-[18px] ml-[11vw] mt-[1vw]" />
                  <Text className=" text-[11px] text-[#0000ff] font-bold flex-auto mt-[-5vw] ml-[17vw]">Sea cucumber rates</Text>
                </View>
              )}
              ...
              {db_type === "News" && (
                <View className="flex mx-[50vw] mb-[5vw] w-[200px] ">
                  <Image source={require("../../assets/notification/rulesAndRegulations.png")} className="w-[15px] h-[18px] ml-[11vw] mt-[1vw]" />
                  <Text className=" text-[11px] text-[#00ff00] font-bold flex-auto mt-[-5vw] ml-[17vw]">News</Text>
                </View>
              )}

              {db_postedById === db_id && (
                <View className="flex mt-[2vh] mb-[2vh]">
                  <TouchableOpacity className="bg-[#C61A1A] rounded-[15px] w-[auto] mx-auto justify-center py-[5px] px-[10px]" onPress={handleDelete}>
                    <Text className="text-[#fff] text-[14px] font-bold text-center">Delete News</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
