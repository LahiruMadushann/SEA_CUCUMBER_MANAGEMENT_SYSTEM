...
import { View, TextInput, Text, ScrollView, Image, TouchableOpacity, Modal, Button } from "react-native";
...
export default function FisheriesFarmingFAQScreen() {
  ...
  const [isMyQuestionsModalVisible, setMyQuestionsModalVisible] = useState(false);
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="bg-[#fff]">
          <View className="absolute w-[223vw] h-[100vh] left-[-62vw] top-[-49vh] bg-[#0013C0]  rounded-b-full ">
            ...
            <View className="mt-[6vh]">
              {category === "fisheries" ? (
                <Text className="text-[22px] text-center font-bold text-[#FFFFFF]">Fisheries Related FAQs</Text>
              ) : category === "aquafarming" ? (
                <Text className="text-[22px] text-center font-bold text-[#FFFFFF]">Aqua Farming Related FAQs</Text>
              ) : null}
            </View>
          </View>
          ...
          <View className="mt-[25vh] mx-auto">
            <View className="flex-row">
              <TouchableOpacity onPress={toggleModal}>
                <View className="w-[auto] h-[auto] pb-1 pt-1 pr-5 pl-5 mt-[2vh] mr-[1vw] bg-[#FFFFFF] mb-[5vh] rounded-full">
                  <Text className="text-[14px] text-center font-bold text-[#5A73F4]">Post your Questions here</Text>
                </View>
              </TouchableOpacity>
              ...
              <TouchableOpacity onPress={toggleModalViewMyQuestions}>
                <View className="w-[auto] h-[auto] pb-1 pt-1 pr-5 pl-5 mt-[2vh] ml-[1vw] bg-[#FFFFFF] mb-[5vh] rounded-full">
                  <Text className="text-[14px] text-center font-bold text-[#5A73F4]">My Questions</Text>
                </View>
              </TouchableOpacity>
            </View>
            ...
            <Modal animationType="slide" transparent={true} visible={isModalVisible} onRequestClose={toggleModal}>
              ...
              >
                ...
                <View className="flex-row mb-4 mt-4 justify-end">
                  <Button title="Close" onPress={toggleModal} className="flex rounded-[5px] w-[auto] h-[auto] p-1 bg-grey-500" />

                  <View className="ml-3"></View>

                  <Button className="flex rounded-[5px] w-[auto] h-[auto] p-1 bg-blue-800 " onPress={handleQuestionSubmit} title="Submit" />
                </View>
              </View>
            </Modal>

            {/* The Modal For viewing your questions*/}
            <Modal animationType="slide" transparent={true} visible={isMyQuestionsModalVisible} onRequestClose={toggleModalViewMyQuestions}>
              <View
                className="flex h-[auto] my-auto mx-auto p-5 rounded-[10px] shadow-lg shadow-gray-900"
                style={{ backgroundColor: "rgba(255, 255, 255, 0.95)" }}
              >
                {allFAQDetails.map((faq) =>
                  faq.category === category && faq.questionAskedByID === db_id ? (
                    <TouchableOpacity onPress={() => setSelectedFAQ(faq)} className="w-[auto] h-[auto] bg-[#FFFFFF] shadow-lg shadow-gray-700 mb-2">
                      <View key={faq._id}>
                        <View className="w-[auto] h-[auto] mr-[5vw] ml-[5vw] mt-[2vw] mb-[2vw] flex-row ">
                          <Text className="text-[14px] font-bold text-[#000000]">
                            {faq.question}? {!faq.answer && <Text style={{ color: "red" }}>Not Answered</Text>}
                          </Text>
                        </View>

                        {selectedFAQ === faq && faq.answer && (
                          <View className="flex mt-[1vw] ml-[5vw] mr-[5vw]">
                            <Text className="text-[12px] text-justify flex mt-[0vw] mb-[4vw]">{faq.answer}</Text>
                          </View>
                        )}
                      </View>
                    </TouchableOpacity>
                ...
                <View className="flex-row mb-4 mt-4 h-auto justify-end">
                  <Button title="Close" onPress={toggleModalclose} className="flex rounded-[5px] w-[auto] h-[auto] p-1 bg-grey-500" />
                </View>
              </View>
            </Modal>
            ...
            {allFAQDetails.map((faq) =>
              faq.category === category && faq.visibleToAll ? (
                <TouchableOpacity onPress={() => setSelectedFAQ(faq)} className="w-[82vw] h-[auto] bg-[#FFFFFF] shadow-lg shadow-gray-700 mb-2">
                  <View key={faq._id}>
                    <View className="w-[auto] h-[auto] mr-[5vw] ml-[5vw] mt-[2vw] mb-[2vw] flex-row ">
                      <Text className="text-[14px] font-bold text-[#000000]">{faq.question}</Text>
                    </View>
                    {selectedFAQ === faq && (
                      <View className="flex-auto mt-[1vw] ml-[5vw] mr-[5vw] mb-[5vw]">
                        <Text className="text-[12px] text-justify flex-auto mt-[0vw]">{faq.answer}</Text>
                      </View>
                    )}
                  </View>
                </TouchableOpacity>
