...
export default function SingleAdvertisementScreen() {
  ...
  useEffect(() => {
    ...
    async function fetchSingleAdvertisementData() {
      try {
        const response = await axios.post(`${BASE_URL}/user/getSingleAdvertisement`, { advertisementId: advertisementId });
  ...
  const handleDelete = async () => {
    ...
    Alert.alert("Are you sure?", "Once you delete your advertisement, you won't be able to recover it.", [
      {
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="bg-[#fff]">
          ...
          <View className="mt-[20vh] mx-auto">
            ...
            <View className="w-[82vw] h-[auto] rounded-[30px] bg-[#FFFFFF] shadow-lg shadow-gray-700 mb-2">
              ...
              <View className="flex">
                <Text className=" text-[11px] text-justify flex-auto mt-[2vw] ml-[10vw] mr-[10vw]">{db_description}</Text>
              </View>
              ...
              {db_postedById === db_id && (
                <View className="flex mt-[2vh] mb-[2vh]">
                  <TouchableOpacity className="bg-[#C61A1A] rounded-[15px] w-[auto] mx-auto justify-center py-[5px] px-[10px]" onPress={handleDelete}>
                    <Text className="text-[#fff] text-[14px] font-bold text-center">Delete Advertisement</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
