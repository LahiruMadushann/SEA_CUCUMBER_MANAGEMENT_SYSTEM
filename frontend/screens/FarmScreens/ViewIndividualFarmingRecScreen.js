...
export default function ViewIndividualFarmingRecScreen() {
  ...
  useEffect(() => {
    ...
    async function fetchAllStockData() {
      try {
        const response = await axios.post(`${BASE_URL}/districtAquaCulturist/getFarmingDetailsFromId`, { farmingId: farmingId });
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="bg-[#fff]">
          <View className="absolute w-[223vw] h-[80vh] left-[-62vw] top-[-49vh] bg-[#0013C0]  rounded-b-full ">
            <View className="mt-[58vh] ">
              ...
              <Text className="text-center text-[#fff]  text-[18px] mt-[2vw] fixed">Stocking Date : {formatDate(db_stockingDates)}</Text>
            </View>
          </View>
