...
import { StyleSheet, Text, View, Image, SafeAreaView, TouchableOpacity, ScrollView, FlatList, Button, Modal } from "react-native";
...
export default function ViewSingleFishingRecScreen() {
  ...
  const handleDelete = async () => {
    Alert.alert("Are you sure?", "Once you delete the record , you won't be able to recover it.", [
      {
        text: "Delete Record",
        onPress: () => {
          const recordData = {
  ...
  useEffect(() => {
    ...
    async function fetchSingleFishingDetails() {
      try {
        const response = await axios.post(`${BASE_URL}/fisherman/getSingleFishingDetails`, { fishingId: fishingId });
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="bg-[#fff]">
            ...
            <View className="mt-[58vh] ">
              <View className="flex-row ">
                <View className=" ml-[4vw]">
                  <TouchableOpacity onPress={() => navigation.navigate("ViewAllFishingRecordsScreen")}>
              ...
              <Text className="text-center text-[#fff] font-bold text-[22px] mt-[10vw] fixed">Fishing Catch Records</Text>
              <Text className="text-center text-[#fff]  text-[18px] mt-[2vw] fixed">
                {db_firstName} {db_lastName}
              </Text>
              <Text className="text-center text-[#fff]  text-[18px] mt-[2vw] fixed">Date : {formatDate(db_date)}</Text>
            </View>
          </View>
          ...
          <View className="mt-[36vh]">
            ...
            <FlatList
              ...
              renderItem={({ item }) => <TableRow label={item.label} value={item.value} />}
              keyExtractor={(item) => item.label}
            />
          </View>
          <View className="flex m-[auto] mt-[2vh]">
            <Image source={{ uri: stockImageUrl }} className=" w-[80vw] h-[30vh] rounded-[2px] bg-[#FFFFFF] shadow-lg shadow-gray-800" />
          </View>
          <View className="flex-row mb-[2vh] mr-[5vw] mt-[5vw] justify-end">
            <TouchableOpacity className="bg-[#D23434] rounded-[5px] w-[40vw] py-[5px] px-[10px] shadow-sm shadow-gray-700" onPress={handleDelete}>
              <Text className="text-[#fff] text-[15px] font-bold text-center">Delete Record</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
