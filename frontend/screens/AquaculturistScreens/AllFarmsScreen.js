...
import { View, Text, TouchableOpacity, Image, TextInput, ScrollView, StyleSheet, Button, Linking } from "react-native";
...
export default function AllFarmsScreen() {
  ...
  useEffect(() => {
    ...
    async function fetchAllFarmData() {
      try {
        const response = await axios.get(`${BASE_URL}/districtAquaCulturist/getAllAquaFarmDetails`);
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="bg-[#fff]">
          <View className="absolute w-[223vw] h-[100vh] left-[-62vw] top-[-49vh] bg-[#0013C0]  rounded-b-full ">
            <View className="mt-[58vh] ">
              ...
              <Text className="text-center text-[#fff] font-bold text-[22px] mt-[10vw] fixed">All Aquaculture Farms</Text>
                ...
                >
                  <View key={item._id}>
                    <View className="w-[auto] h-[25px] ml-[5vw] mt-[4vw] flex-row ">
                      <Text className="text-[18px] font-bold text-[#5A73F4]">{item.name}</Text>
                    </View>

                    <View className="flex mt-[1vw] ml-[10vw]">
                      <Text className=" text-[15px] flex-auto mt-[1vw]">Location : {item.location}</Text>
                      {item.stock && <Text className=" text-[15px] flex-auto mt-[1vw] ">Total Stock : {item.stock}</Text>}
                      {!item.stock && <Text className=" text-[15px] flex-auto mt-[1vw] ">No Stock available</Text>}
                      {item.stock && (
                        <Text className=" text-[15px] flex-auto mt-[1vw]">Last stock update : {`${formatDate(item.stockingDates)}`}</Text>
                      )}
                    </View>
                  </View>
                </TouchableOpacity>
